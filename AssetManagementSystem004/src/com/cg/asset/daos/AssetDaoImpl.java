...
public class AssetDaoImpl implements IAssetDao {
	...
	public List<RequestAndAssetQty> getRequestsPendingListWithQty() throws AssetException {
		String query = "select r.requestid,r.empno,r.mgrId,r.assetid,r.requestdate,r.requestfordays,r.status,a.quantity from request r,asset a where r.status = ? and a.assetid = r.assetid";
		List<RequestAndAssetQty> requestList = new ArrayList<RequestAndAssetQty>();
		try {
			pst = con.prepareStatement(query);
			pst.setString(1, "Pending");
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
				...
				RequestAndAssetQty request = new RequestAndAssetQty(requestId,mgrId,empNo,assetId,date,requestForDays,status,qty);
				requestList.add(request);
